<template>
  ...
  <div v-if="!submitted">
    <form class="mt-10 ml-10 mr-10 basis-[1000px] max-w-[1000px] ">
      <h2 class="text-lg text-center font-bold mb-4 text-blue-700">Edit Package</h2>
      <div>
        <label for="packageName" class="block text-sm font-medium text-gray-700">Package Name</label>
        <input v-model="forfait.packageName" type="text" name="packageName" id="packageName" placeholder="Package Name"
      ...
      <div class="flex gap-4">
        <button type="button" @click="editForfait" class="bg-blue-500 text-white px-4 py-2 rounded">Save</button>
      </div>
...
export default {
  props: ['addFor', 'updateFor', 'destinations'],
  data () {
    return {
      message: null,
      submitted: false,
    ...
    editForfait () {
      ForfaitDataService.update(this.id, this.forfait)
        .then((response) => {
          ...
          this.message = null
          this.submitted = true
          const index = this.destinations.findIndex(f => f.id === this.id)
          if (index !== -1) {
            this.updateFor(index, this.forfait)
          }
